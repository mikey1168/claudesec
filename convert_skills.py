...
Specific parser for Markdown format with ## headers and - **Field**: value
...
def parse_skill(content: str, file_path: Path) -> dict:
    ...
    meta = {
        'name': None,  # Will be equal to folder
        'folder': None,
        'source': None,
        'description': [],
        'triggers': [],
    ...
    for line in lines:
        ...
        if not stripped:
            if state in ('DESCRIPTION', 'BODY'):
                # Preserve line breaks in description and body
        ...
        # Detect Markdown headers (## Section)
        ...
        if state == 'METADATA' and stripped.startswith('- **'):
            ...
            if match:
                ...
                if key == 'skill_name':
                    pass  # We don't use this for name
                elif key == 'folder':
                    meta['folder'] = val
                    # name = folder (per requirement)
                    meta['name'] = re.sub(r'[^a-z0-9]+', '-', val.lower()).strip('-')
                elif key == 'source':
                    meta['source'] = val
        
        elif state == 'DESCRIPTION':
            # Ignore lines that are headers or metadata
            if not stripped.startswith('##') and not stripped.startswith('- **'):
                meta['description'].append(stripped)
        
        elif state == 'TRIGGER':
            # Look for block between backticks: `a, b, c`
    ...
    # Fallback: if no folder, use the file's parent folder name
    if not meta['name']:
        parent = file_path.parent.name
        meta['name'] = re.sub(r'[^a-z0-9]+', '-', parent.lower()).strip('-') if parent.lower() != 'skills' else file_path.stem
...
def main():
    print("🚀 Claude Skills Converter v4 - Markdown Format ##\n")
    ...
    # Debug mode: process only 1 file
    import sys
    ...
    if ok:
        ...
        print(f"\n📋 To upload to Claude Console:")
        print(f"   • Each ZIP contains: [folder-name]/SKILL.md")
        print(f"   • The 'name' in YAML = name of the folder inside the ZIP")
    else:
        print(f"\n❌ Could not convert any skill. Run with --debug to diagnose.")
