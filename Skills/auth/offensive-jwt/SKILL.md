...
- **Algorithm Confusion**: Change `alg` to `none` (or `None`, `NONE`).
- **HMAC vs RSA**: Change `RS256` to `HS256` and sign the token using the public key as the secret.
- **Header Injection (JKU/JWK)**: Inject a custom `jku` (JWK Set URL) pointing to your own controlled public key.
- **KID Manipulation**: Try `kid: ../../../../../../dev/null` or `kid: "command injection"`
