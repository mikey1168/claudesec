...
- Source: https://github.com/sahar042/offensive-checklist/blob/main/active-directory.md


## Advanced Bypass Techniques & Payloads

- **AMSI Bypass**: Patch `AmsiScanBuffer` in memory using PowerShell or C#.
- **ETW Patching**: Disable Event Tracing for Windows by patching `EtwEventWrite`.
- **LOLBins**: Use built-in tools like `certutil.exe` or `msbuild.exe` to execute payloads instead of raw PowerShell.
